...
import fol.{FolParser, folFormula}
...
object bpmnLoader {
  ...
  def goalsFromInputStream(is : InputStream) : String = {
    ...
    if (w.isDefined) {
      ...
      for (i <- workflow.items if (i.isInstanceOf[Task])) {
        ...
        goal_string += "\tpre: " + pre.prettyString() +"\n"
        goal_string += "\tpost: " + post.prettyString() +"\n\n"
  ...
  def fullFromInputStream(is : InputStream) : String = {
    ...
    if (w.isDefined) {
      ...
      for (i <- workflow.items if (i.isInstanceOf[Task])) {
        ...
        goal_string += "GOAL: " + task.label.replace("\n", " ")+"\n"
        goal_string += "\tws: " + ws.prettyString() +"\n"
        goal_string += "\tgs: " + gs.prettyString() +"\n"
        goal_string += "\tpre-inf: " + pre_inf.prettyString() +"\n"
        goal_string += "\tpost-inf: " + post_inf.prettyString() +"\n"
        goal_string += "\tpre-cond: " + pre.prettyString() +"\n"
        goal_string += "\tfinal-state: " + post.prettyString() +"\n\n"
  ...
  private def extract_workflow(node: Elem) = {
    if (node.label == "definitions") {
      ...
      Some(Workflow(datatypes, items.to[ArrayBuffer], flows.to[ArrayBuffer], data.to[ArrayBuffer]))
  ...
  private def parse_sequence_flow(nodes: NodeSeq) : List[SequenceFlow] = {
    ...
    for (p <- nodes) {
      ...
      var optional_condition : Option[folFormula] = None
