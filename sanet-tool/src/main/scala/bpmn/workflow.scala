...
case class SequenceFlow(id: String, start : Item, end : Item, condition : Option[folFormula]) extends Flow(id)
...
case class Workflow(datatypes : ArrayBuffer[DataType], items : ArrayBuffer[Item], flows : ArrayBuffer[Flow], data : ArrayBuffer[Data]) {
  ...
  def control_flows : ArrayBuffer[Flow] = for (f<-flows if f.isInstanceOf[SequenceFlow]) yield f

  def outgoing_flows(item : Item) : ArrayBuffer[Flow] = {
    var sel_flows : ArrayBuffer[Flow] = ArrayBuffer()
